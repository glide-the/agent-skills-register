...
4. **追加 Lock File 格式统一设计**——加入：
 ...
 Created [](file:///Users/dmeck/project/agent-skills-register/docs/design/feasibility-plan.md)

Created 4 todos

已生成完整的可行性方案文档 feasibility-plan.md，包含全部要求的章节和附录：

**第一章：三维能力兼容矩阵** — 按 Registry（14 项）、CLI 客户端（12 项）、Skill Store（14 项）三个维度列出了 40 个需求子项的覆盖度对照表。

**第二章：三套互斥实现方案**，各含数据流（mermaid 图）、优劣势、工作量和成本估算：
- **方案 A（ClawHub-First）**：Fork ClawHub，Convex 生态内扩展，10-16 人月
- **方案 B（自建 Registry）**：PostgreSQL+S3 独立构建，双 CLI 协议兼容，18-27 人月
- **方案 C（轻量混合）**：Git 存储 + 元数据服务 + vercel-skills Fork，9-15 人月

**第三章：决策矩阵** — 8 维加权评分，**推荐方案 B**（自建 Registry），核心理由是安全合规从底层设计 + 平台无锁定 + 双生态兼容。建议 Phase 0 用方案 C 的轻量模式快速验证。

**第四章：四阶段路线图**（Phase 0-3，14 个月），含 Gantt 图、每阶段交付物和风险缓解。

**第五章：风险登记簿** — Top 15 风险，含影响/概率/缓解措施/责任人。

**附录 A**：34 个 API 端点的完整兼容映射表（1:1 兼容/适配转换/新增/延迟/不实现）。

**附录 B**：统一 Lock File JSON Schema（Global v4 + Local v2），兼容 ClawHub `.clawhub/origin.json` 和 vercel-skills 两层 Lock。
